...
seed/              规范（决策文档，入口 seed/README.md；00–06）
agents/            九个角色的指令（路由启动子 agent 时使用）
...
带图和小例子的完整版见 [docs/工作流程.md](docs/工作流程.md)。


```
讲解写故事 → 人走故事、理解一致
→ 业务分析补「使用」语句（会不会同时 / 重复 / 一次几条 / 失败处置 / 谁能看见）→ 人确认
→ 模型师建最少模型、走故事、列选择 → 校验 ①（机械：每条 G / R / U 都有落点）
→ 讲解出题 → 人预测、过裁定卡 → 写回 → 回流 → 校验角色填判断 → 人审阅 → 模型确认
→ 路由算编码计划（要动哪些文件、什么顺序）→ 原型补关键逻辑 → 人确认计划
...
→ 路由算外壳计划 → 编码补关键逻辑（事务、锁、状态码）→ 人确认
→ 编码写外壳与外壳测试（领域代码一行不动）→ plan check → 校验 ②（仍 0 差异）
→ pre-pr 审查（C 契约完整 / E 防御正确性 / F 并发与状态 / S 风格）→ 人审阅 → 合并
```

既有代码用 `read` 让解读角色先还原成业务描述与模型草稿。`slice next` 随时能算出当前卡在哪、下一步该谁；`board` 看全部切片。

## 三个让 agent 少出错的位置

- **规范**：`seed/03` 管能不能解码（硬），`seed/06` 管读着顺不顺（软）。
- **上下文**：故事（人物、日期、金额）+ 模型 + 裁定 + 使用语句。
...
node tools/story.js usage <项目> <id> propose <U-…|--none> | confirm   # 使用语句：登记 / 人确认
...
| 校验器（机械检查 + 判断清单 + 报告） | 完成（方向 ① 含使用语句的落点与判断；方向 ② 解码 + 比对 + lint） |
| 审阅工具 | 完成（校验报告与 pre-pr 报告通用） |
| 路由 + 九个角色指令 | 完成（`SKILL.md` 的 `run` 一次推进一步；`agents/` 九份） |
| 切片驱动 | 完成（`slice.js`：new / next / advance / apply / log；`story.js` 含使用语句登记与确认） |
