...
<project>/                    每个项目一个文件夹、一个 git 仓库；变化靠 git 追溯
  ...
  plans/<id>.json             编码计划：这次要动哪些代码文件、按什么顺序、每步守什么（路由算骨架，写码角色补关键逻辑，人确认）
...
陈述性语句，不写故事、步骤、角色矩阵。三类语句，每条带编号，编号一经分配永不复用：
...
- [U-003] (使用) 一次导入最多 200 条发票，其中一条信息不全，其余照常入账
```

目标与规则说的是**一次操作对不对**；使用说的是**这个系统会被怎么用**——会不会同时、会不会重复、一次几条与部分失败算什么、失败了业务上怎么处置、谁在什么时候能看见。这类语句通常不在原料里，要业务分析主动问出来；模型必须回应它们（同时改 → 版本号或状态守卫；重复 → 状态守卫或幂等；一次几条 → 命令的输入形状与部分失败的语义；失败处置 → 事件或错误；可见性 → 查询的范围），校验 ① 会要求每条使用语句都有落点。
...
| 规则・反应 | 事件处理 |
| 规则・推导 | 值对象或行为的计算 |
| 使用 | 行为的守卫或规则、命令 / 查询的输入形状、事件或错误——不限种类，但必须有 |
