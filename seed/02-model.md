...
<project>/                    每个项目一个文件夹、一个 git 仓库；变化靠 git 追溯
  ...
  business/00-全景.md         粗读的产物：这门生意、钱的路径、模块候选；给人读，不进解析
  business/<Module>/业务抽象.md   第一层：手册层面的抽象业务（按段落点亮）
  business/<Module>/业务落地.md   第二层：这家公司的做法，含五问问出来的公司事实
  glossary.json               词汇表
...
分几层、怎么判、怎么写、怎么点亮，全在 [07-business-layers.md](07-business-layers.md)。这里只记形状：陈述性语句，不写故事、步骤、角色矩阵。两类编号（能力 G、规则 R），每条带编号与 `(层-种类)` 标签，编号按点亮顺序发、永不复用。`业务抽象.md` 与 `业务落地.md` 都按能力 / 规则分节：
...
- [R-004] (业务落地-事实) 同一份预算可能由两位协调员先后经手
```

校验器只认以 `- [G-`、`- [R-` 开头的行（`- [U-` 是老项目里已停发的使用语句，只认不发）；语句下面缩进的依据子项（出处、批次）不是语句。种类五种：能力、事实、约束、公式、触发（定义与判据见 07 第三节）。

五问（会不会同时、会不会重复、一次几条与部分失败、失败了业务上怎么处置、谁在什么时候能看见）问出来的公司事实通常不在原料里，要业务分析主动问出来、写成普通的业务落地语句；模型师照它们定第三层的回应（同时改 → 版本号或状态守卫；重复 → 状态守卫或幂等；一次几条 → 命令的输入形状与部分失败的语义；失败处置 → 事件或错误；可见性 → 查询的范围），写在元素规则里，校验 ① 会要求每条语句都有落点。
