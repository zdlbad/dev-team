...
| `workbench <项目> [--code <代码库>] [--port 4870]` | **工作台**：一个地址、一套页签（谁在干什么 · 等你答 · 走故事 · 模型图 · 这段改了什么 · 审模型 · 审代码 · 计划 · 原型），自己把故事 4871、原型 4872、现场 4873、审阅 4874、代码审查 4876 拉起来（端口占着就复用），增量页与计划按当前段落现算。**开工第一件事就是把它起来**，人随时能查 | `node $DEV_TEAM/tools/workbench.js …` |
