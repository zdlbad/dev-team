---
name: dev-team
description: 模型驱动的开发团队。业务 → 模型 → 原型 → 代码，模型可编码、代码可解码，校验互证；七个角色（业务分析、讲解、模型师、原型、编码、模型校验、解读）按故事切片迭代，每条故事都能在原型上按着走，人在每个门禁拍板。用法：/dev-team <子命令> …
---
...
| `proto serve <项目> --code <代码库> [--port 4872]` | 起原型页面：编译代码库、启动内存原型；按故事走（每步用模型师 / 原型填的输入跑，和故事的事实并排）、手动跑命令与查询、看聚合状态与事件流水、重置、重新编译 | `node $DEV_TEAM/tools/proto.js serve …` |
| `proto check <项目> --code <代码库>` | 编译 + 启动 + 对照模型：命令、查询、聚合是否都登记进原型；故事的步骤能否对上 | `node $DEV_TEAM/tools/proto.js check …` |
| `inventory <代码库> [--out <md>]` | 盘点一个既有代码库：规模、目录、疑似入口、疑似持久化、目录间引用、每个文件的符号（给解读角色的地图） | `node $DEV_TEAM/tools/inventory.js …` |
...
2. 按 `role` 分派：
   ...
   - **业务分析 / 模型师 / 原型 / 编码**：读对应的 `agents/*.md`（原型是 `agents/prototyper.md`），同样方式启动子 agent，任务 = `action`。子 agent 完成后，把它的「产出」与「问题清单」原样转给人，停。**不替人回答问题清单**。原型完成后另起 `proto serve` 页面，把地址告诉人。
...
切片分两种：**故事切片**（一条故事：业务理解一致 → 最少模型 → 走故事、裁定 → 模型确认 → 原型可按）与**实现切片**（若干条已在原型上走通的故事 → 生产外壳 → 校验 ②）。故事管业务到模型到原型；生产代码的范围另定，几条故事攒够了一起上。

战略设计（模块划分）通盘做一次；之后**每个建模切片是一条能走通的故事**，不是一组主题。`slice new … --story` 建切片，`slice next` 会依次要求：讲解写故事 → 人走故事、业务理解一致 → 模型师建最少的模型并走故事、列选择 → 校验 ① 机械检查 → 讲解出题 → 人走故事、过裁定卡（`story serve`）→ 路由写回（`story apply`）→ 回流模型师 → 校验角色填判断 → 人审阅 → 模型确认 → **原型**（领域代码 + 内存适配器，故事能在页面上按着走）→ 校验 ②（解码比对）→ 人在原型上走故事 → 合并。**每条故事都反映在原型上**：原型里的领域代码就是最终代码，只有仓储与端口是内存的。生产的外壳（数据库、接口、界面）由实现切片补：`slice new … --implements s-002,s-003` → 编码 → 校验 ②。理由见 `seed/01-phases-and-slices.md`「故事切片」。
...
3. 完成后把「这个系统是什么」那段、产出与问题清单原样转给人；之后进正常流程：人读业务描述 → 讲解写导读与故事 → 模型师在草稿上按故事收敛。
