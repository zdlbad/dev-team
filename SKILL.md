...
人愿意的话可以**直接在现场页面上答**，不用你转述：页面把还没答的问题摆在最前面，点一个答案、想补充就写几句、按「答这个」。他在页面上答了，`reports/_scene.json` 里就有，你 `scene questions` 看得到（`answeredVia` 写着「网页」）。**答过的问题页面上还留着，点「改一下」能改**——他点错过一次；改了旧答复进 `answerHistory` 不抹掉，你看见 `answerHistory` 非空就要想一想：那个角色是不是已经照旧答复做过了，做过就把它叫回来返工。切换只影响还没问出口的问题，已经挂着的照旧等他答。
