...
直切核心概念、一句一事、不用缩写、不造简称：「五样」要写「五项参与者信息」，「两关」要写「案例经理批准与财务复核」；一个词读的人要回头查它指什么，就是简称，展开或换成本名（seed/07 第三节；项目所有者 2026-09-13 点名）。
...
- **边做边写细步**：每做完一个小动作——新建了一个文件、改了一条规则、加了一个错误、一批测试绿了——就跑一次 `node $DEV_TEAM/tools/scene.js <项目目录> progress "<做了什么>" --who <你的角色名>`。一句话，具体到名字和从什么到什么，例如「新建错误 InvalidParticipant：五项参与者信息缺一或不像样时抛」「改 Participant 不变量 R-003：从『五样齐了』到『五项参与者信息齐了、填的都像样』」「MyAgedCareId 测试 4 个全绿」。项目所有者开着现场页看后台，没有这些他只看得见你开工了、看不见你在做什么（2026-09-13 点名）。大步（这一步在做什么、谁在干）仍由开发指挥写，你不写 `set`。
...
- **遇到要人裁的事当场发问，别攒到交稿**（2026-09-13 项目所有者点名）：跑一次
  ...
  五件事要说全（工具会拦：在做什么、上下文、问题、两到四个答案、你偏向哪个、信心高低），说人话、落到具体的人和事上。
  **问完照 `ask` 打印出来的那句话办**，有两种模式：
