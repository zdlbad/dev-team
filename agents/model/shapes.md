...
**创建**（`create`）：`{ "purpose", "input?", "steps", "rules", "throws", "traces" }`，与行为同一套，只是**不发事件**——创建落在静态工厂里，编码规范不许工厂 `raise`，解码器也读不出来。建出来这件事要让别的聚合或模块知道，由命令的下一步去做（调一个行为、经端口交过去）。一件东西怎么被建出来写在这里；时时都要成立的约束留在 `invariants`。老模型没有 `create`、只有不变量的照样认。
...
不记发布方。谁发谁记（行为与领域服务操作的 `raises` / `throws`，创建 `create` 只有 `throws`）；校验反向核对每个事件、每个错误至少有一个发布方。
