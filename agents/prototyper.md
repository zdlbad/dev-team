...
1. **补关键逻辑**：读 `plans/<id>.md`。每个标「待补」的步骤，在 `keyLogic` 里写这一步守哪条规则、哪条不变量在这里生效、`when` 分流怎么走、哪条五问的回应在这里落地（比如「连点两次确认，第二次在 status 守卫处抛 OrderFailed」）。**补完停下**，等人 `plan confirm`。
  ...
  - 字数按 JavaScript 字符数量，空格与换行都算：**不要为了对齐编号补空格**，一对齐字数就翻倍（2026-09-13 s-001 第一版 575 / 615 字，去掉对齐空格才过）。
...
- 开写前读计划里「人在步骤上留的话」（`plans/<id>.md` 那一节，或步骤的 `humanNotes`）：人对哪一步有话就按话改，`plan done` 的说明里回一句改了什么
- 其它注释按 03「注释写什么」：讲代码的行为和原因；类上只说这个类守的规则，要提示别处守的最多一句「唯一性不由这里守」；上下文不必写全，模型的 `note` 不照抄进代码（第八十七批）
