...
1. **补关键逻辑**：读 `plans/<id>.md`。每个标「待补」的步骤，在 `keyLogic` 里写这一步守哪条规则、哪条不变量在这里生效、`when` 分流怎么走、哪条五问的回应在这里落地（比如「连点两次确认，第二次在 status 守卫处抛 OrderFailed」）。**补完停下**，等人 `plan confirm`。
   **关键逻辑的写法**：一步不超过 300 字（节奏门禁，超了 `plan confirm` 会拒收）；写成代码风格的多行文本，不写段落——守卫按执行顺序一行一条，结果写 `→ throw X` / `→ return` / `→ raise E`，赋值一行一个，编号放行尾 `//` 后；开头一行「改动：」说这一步改什么，结尾「不动：」列不碰的；**不复述模型规则原文**（人已经确认过模型），只写这一步的落点、顺序与分流。
...
8. **最后跑** `node $DEV_TEAM/tools/plan.js check <项目> <切片> --code <代码库>`（文件都在、顺序一致、`walk.input` 齐）与 `node $DEV_TEAM/tools/proto.js check <项目> --code <代码库>`（模型里的命令、查询、聚合都登记；先跑本故事线前面的段落，再跑本段，每个有动作的步骤都能对上）。前面段落的老路被本段弄断了，报告，不自己改模型。缺的补，补不了的写进问题清单。
