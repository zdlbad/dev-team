...
/** 七个角度。A–D 对着模型与故事查（模型说的代码做了没）；E–F 对着代码本身查（模型不表达的：输入边界、错误处理、并发）；S 对着 seed/06 查风格，两种模式都跑，只出说明 / 应该改。 */
const ANGLES = {
  ...
  'B 被删的不变量': { question: '这次改动有没有删掉或放松一条规则，模型也同步删了，但没有人拍板？', pass: '每条模型 throws / raises / rules 在聚合行为里有守卫，且有一个测试用例走到它；这次改动删掉的守卫都能在裁定文件里找到依据。', fail: '守卫没了或条件变宽了，模型跟着改了所以解码比对是干净的，但 raw/项目所有者的裁定.md 里没有这条。', how: 'git diff 看这次动过的聚合行为；每个被删或被改的 if 找裁定；每个现存的 throws / raises 找测试。' },
